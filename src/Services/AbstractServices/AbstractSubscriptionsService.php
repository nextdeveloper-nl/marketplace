...
class AbstractSubscriptionsService
{
    public static function get(SubscriptionsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new SubscriptionsQueryFilter($request);
        }
    ...
    {
        $object = Subscriptions::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Marketplace\\Actions\\Subscriptions\\' . Str::studly($action);
    ...
    {
        try {
            $obj = Subscriptions::where('uuid', $uuid)->first();
